...
/*char getNamebyID(int a){
    char buffer[10];
    //char str[5]="V"
    itoa(a, buffer, 10);
    
    //return strcat(str,buffer);
    return buffer;
}*/
...
int main() {
    ...
    while (!feof(fp)) {
        ...
        addVertex(graph, castleID1, getNamebyID(castleID1));
        addVertex(graph, castleID2, getNamebyID(castleID2));
        ...
        if( time>=50 ){
            addVertex(walkingGraph, castleID1, getNamebyID(castleID1));
            addVertex(walkingGraph, castleID2, getNamebyID(castleID2));
    ...
    while (choice != 7) {
        ...
        case 4:
            ...
            int Max = indegree(graph, 1, output); 
            
            for (i = 2; i <= ROW; i++){
                if (indegree(graph, i, output) <= Max) continue;
                else Max = indegree(graph, i, output);
            }
            
            for (i = 1; i <= ROW; i++) {
                if (indegree(graph, i, output) == Max) {
                    printf("%d ", i);
                }
            }
            printf("\n");
